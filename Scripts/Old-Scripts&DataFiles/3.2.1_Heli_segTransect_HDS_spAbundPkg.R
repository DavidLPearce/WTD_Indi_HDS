...
transect_effort <- transect_effort[,-7]
...
# Getting number of segments original transect was broken into
print(transects)
# 
# segT_tot <- sum(
#                 1 * 5 + 
...
fall23_mat <- matrix(0, nrow = segT_tot, ncol = 4)
win24_mat <- matrix(0, nrow = segT_tot, ncol = 4)
fall24_mat <- matrix(0, nrow = segT_tot, ncol = 4)
win25_mat <- matrix(0, nrow = segT_tot, ncol = 4)

# Define Transect IDs (1 to 12) and Survey Codes (1 for Evening, 2 for Morning)
transect_ids <- 1:segT_tot   # Each transect appears twice
